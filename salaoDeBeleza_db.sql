...
CREATE TABLE IF NOT EXISTS Agendamento (
  ...
  CONSTRAINT fk_Agendamento_Cliente1
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT fk_Agendamento_Colaborador1
...
CREATE TABLE IF NOT EXISTS Produto_has_Servico (
  Produto_idProduto INT NOT NULL,
  Servico_idServico INT NOT NULL,
  quantidadeProdutoUtilizado INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS Servico_has_Campanha (
  Servico_idServico INT NOT NULL,
  Campanha_idCampanha INT NOT NULL,
  PRIMARY KEY (Servico_idServico, Campanha_idCampanha),
  CONSTRAINT fk_Servico_has_Campanha_Servico1
    FOREIGN KEY (Servico_idServico)
    REFERENCES Servico (idServico)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT fk_Servico_has_Campanha_Campanha1
    ...
    DELIMITER $
CREATE TRIGGER tg_agendamento_ai AFTER INSERT
ON agendamento FOR EACH ROW
BEGIN
DECLARE idProdutoProcurado INT;
DECLARE idServicoProcurado INT;
DECLARE quantidadeProdutoUtilizadoProcurado INT;
DECLARE Servico_idServico_produto_has_servico INT;
DECLARE terminou BOOLEAN DEFAULT FALSE;
DECLARE cursor_produto_has_servico CURSOR FOR SELECT Produto_idProduto, Servico_idServico, quantidadeProdutoUtilizado FROM produto_has_servico;
DECLARE CONTINUE HANDLER FOR NOT FOUND SET terminou = TRUE;
OPEN cursor_produto_has_servico;
loop_produto_has_servico: LOOP
FETCH cursor_produto_has_servico INTO idProdutoProcurado, idServicoProcurado, quantidadeProdutoUtilizadoProcurado;
IF terminou THEN
LEAVE loop_produto_has_servico;
END IF;
BEGIN
update produto set quantidadeEstoque = quantidadeEstoque - quantidadeProdutoUtilizadoProcurado where new.Servico_idServico = idServicoProcurado and idProduto = idProdutoProcurado;
END;
END LOOP;
CLOSE cursor_produto_has_servico; 
END $

CREATE TRIGGER tg_agendamento_ad AFTER DELETE
ON agendamento FOR EACH ROW
BEGIN
DECLARE idProdutoProcurado INT;
DECLARE idServicoProcurado INT;
...
END IF;
BEGIN
update produto set quantidadeEstoque = quantidadeEstoque + quantidadeProdutoUtilizadoProcurado where old.Servico_idServico = idServicoProcurado and idProduto = idProdutoProcurado;
END;
END LOOP;
CLOSE cursor_produto_has_servico; 
END $
...
insert into endereco values (null, 29600000, 'Avenida Brasil', 100, 'Centro', null, 1);
insert into empresa values(13810495000166, 'Eros Salão de Beleza - LTDA', 'Eros Salão de Beleza', 'Lucas', '4436638000', '44997029956', 1);
insert into colaborador values(11581495900, 'Leonardo de Oliveira Klen', 1, '44997029956', 'leoklen', '203', 13810495000166, 1);
insert into servico values(9999, 'Folga', 0);
insert into cliente values(9999, 'Folga', 0, 0, 9999, 'Folga');